...
export class WebMultiProvider implements Provider {
  ...
  public get status() {
    return this.statusTracker.getStatusFromProviderStatuses();
  }
  ...
  private flagResolutionProxy<T extends boolean | string | number | JsonValue>(
    ...
    for (const providerEntry of this.providerEntries) {
      const shouldContinue = this.evaluateProviderEntry(
        flagKey,
        flagType,
        defaultValue,
        providerEntry,
      ...
      if (this.evaluationStrategy.runMode === 'sequential' && !shouldContinue) {
        break;
      }
    }
  ...
  private evaluateProviderEntry<T extends boolean | string | number | JsonValue>(
    ...
    hookHints: HookHints,
    context: EvaluationContext,
    results: ProviderResolutionResult<T>[],
  ) {
  ...
  private evaluateProviderAndHooks<T extends boolean | string | number | JsonValue>(
    ...
    try {
      providerContext = this.hookExecutor.beforeHooks(provider.hooks, hookContext, hookHints);

      evaluationResult = this.callProvider(provider, flagKey, defaultValue, providerContext) as ResolutionDetails<T>;
      ...
      this.hookExecutor.finallyHooks(
        provider.hooks,
        { ...hookContext, context: providerContext ?? hookContext.context },
        hookHints,
      );
  ...
  private callProvider<T extends boolean | string | number | JsonValue>(
    provider: Provider,
    flagKey: string,
    defaultValue: T,
    context: EvaluationContext,
